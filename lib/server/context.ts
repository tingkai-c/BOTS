...
export async function userId(){if(process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY){const session=await auth();if(!session.userId)throw new Error('Sign in to use your shopping agent.');return session.userId;}if(liveMode())throw new Error('Live mode needs Clerk authentication configured.');return 'demo';}
...
export function apiError(error:unknown){return Response.json({error:error instanceof Error?error.message:'Something went wrong. Please try again.'},{status:400});}
