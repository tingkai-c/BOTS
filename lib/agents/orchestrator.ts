...
import { z } from 'zod';
import { identify, model } from '@/lib/ai/identify';
...
export async function orchestrate(state:SearchState,input:SearchInput,user:string,emit:(event:StreamEvent)=>void,signal:AbortSignal){
 ...
 try{await run({marketplace,status:'searching',message:`Searching ${marketplace==='ebay'?'eBay':'Facebook Marketplace'}`});await event(`Opened ${marketplace==='ebay'?'eBay':'Facebook Marketplace'}`,marketplace);
 ...
 else {const profile=await connection(user,marketplace);const session=await createSession(profile?.profileId);sessionId=session.id;await run({marketplace,status:'searching',message:'Browser connected · searching listings',sessionId,debugUrl:viewerUrl(session.debugUrl)});const {browser,page}=await connectBrowser(session.id);try{await event(`Searched “${product.searchQueries[0]}”`,marketplace);await (marketplace==='ebay'?searchEbay:searchFacebookMarketplace)(page,state.id,product.searchQueries[0],input,add);}finally{await browser.close().catch(()=>{});}}
 const count=state.listings.filter(l=>l.marketplace===marketplace).length;await run({marketplace,status:'complete',message:`${count} listings found · session finished`});await event(`${count} listings found on ${marketplace==='ebay'?'eBay':'Facebook'}`,marketplace,'success');return {count};
 ...
 // Deterministic parallel execution guarantees both selected sources run. Model tools
