...
export async function createSession(profileId?:string,persistProfile=false){
  return steel().sessions.create({
    timeout:300000,
    profileId,
    persistProfile,
    useProxy:true,       // routes through Steel's residential proxy pool instead of datacenter IPs
    solveCaptcha:true,   // auto-detects/solves CAPTCHAs instead of silently returning 0 results
  });
}
