...
test('search, inspect, save, negotiate, approve, restore and mobile',async({page})=>{
 ...
 await expect(page.locator('.listing-card')).toHaveCount(9);await page.screenshot({path:'test-results/results-desktop.png',fullPage:true});
 ...
 const url=page.url();await page.reload();await expect(page.locator('.listing-card')).toHaveCount(9);expect(page.url()).toBe(url);
...
test('image upload, identification, connection, empty filters and invalid upload',async({page})=>{
 ...
 await page.getByLabel('Maximum price').fill('1');await expect(page.getByText('No matches just yet.',{exact:true})).toBeVisible();await page.getByRole('button',{name:'Broaden filters'}).click();await expect(page.locator('.listing-card')).toHaveCount(10);
