...
test('ranking uses total landed cost and bounded transparent scores',()=>{const input=searchSchema.parse({query:'Sony WH-1000XM5'});const listings=demoListings('test',input,demoIdentify(input.query));const ranked=rankListings(listings);assert.equal(ranked.length,8);assert.ok(ranked.every(l=>l.dealScore>=0&&l.dealScore<=100));assert.ok(ranked[0].price<ranked.at(-1)!.price);const withShipping={...listings[0],shippingCost:200};assert.ok(rankListings([...listings.slice(1),withShipping]).find(l=>l.id===withShipping.id)!.dealScore<ranked.find(l=>l.id===withShipping.id)!.dealScore);});
