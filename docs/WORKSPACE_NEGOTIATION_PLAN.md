...
- The generic adapter's attribute contract (`data-conversation-id`, `data-message-id`, `data-direction`) matches no real marketplace, so **Facebook now has its own adapter** in `lib/marketplaces/facebook/conversation.ts`. It reaches the conversation only from the listing's own Message affordance, proves the listing association from the item card (or the validated item URL for an inline first contact), derives direction from agreeing layout/avatar/accessibility signals, and derives message identity from direction plus exact text plus repeat index. Availability and currency detection were also corrected: Marketplace labels its buyer action just "Message" on a `role="button"` div, which previously left every Facebook listing as `availability: "unknown"`, and `currencyVerified` is no longer hardcoded true.
- Facebook coverage is `tests/facebook-conversation.test.ts` (transcript classification) and `tests/browser/facebook-chat.spec.ts` (the adapter driven against Marketplace-shaped fixtures in a real browser). eBay and Kijiji still use the generic attribute contract and remain unvalidated. No adapter has been validated against a signed-in live DOM, and no real seller message was sent.
