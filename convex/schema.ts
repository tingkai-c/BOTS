...
export default defineSchema({
 users:defineTable({userId:v.string(),phone:v.optional(v.string())}).index('by_user',['userId']),
 ...
 threads:defineTable({key:v.string(),userId:v.string(),workspaceId:v.string(),listingId:v.string(),listingUrl:v.string(),marketplace,profileId:v.string(),settings:settingsValidator,signature:v.string(),version:v.number(),revision:v.number(),handledRevision:v.number(),status:threadStatus,reason:v.optional(v.string()),conversationKey:v.optional(v.string()),currentOffer:v.optional(v.number()),lastCheckedAt:v.optional(v.number()),createdAt:v.number(),notification:v.optional(v.object({status:v.union(v.literal('sent'),v.literal('skipped')),detail:v.string(),at:v.number()}))}).index('by_key',['key']).index('by_workspace',['workspaceId']).index('by_user_listing',['userId','listingUrl']).index('by_account',['userId','marketplace','profileId']),
